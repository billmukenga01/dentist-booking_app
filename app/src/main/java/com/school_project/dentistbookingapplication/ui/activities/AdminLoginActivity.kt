...
class AdminLoginActivity : BaseActivity(), View.OnClickListener {
            ...
            FirebaseAuth.getInstance().signInWithEmailAndPassword(email, password)
                .addOnCompleteListener { task ->
                    ...
                    if (task.isSuccessful) {
                        FirestoreClass().getUserDetails(this@AdminLoginActivity)
                    } else {
     ...
     */
    fun adminLoggedInSuccess(user: User) {
        ...
        if (user.profileCompleted == 0) {
            ...
            val intent = Intent(this@AdminLoginActivity, AddDoctorActivity::class.java)
            intent.putExtra(Constants.EXTRA_USER_DETAILS, user)
            startActivity(intent)
        } else {
            // Redirect the user to Dashboard Screen after log in.
            startActivity(Intent(this@AdminLoginActivity, AddDoctorActivity::class.java))
        }
        finish()
    }
}